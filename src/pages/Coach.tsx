    ...
    <main className="min-h-screen flex flex-col items-center justify-center p-6 bg-gradient-to-br from-bg-start via-bg-mid to-bg-end font-sans relative overflow-hidden">
      ...
      <div className="relative max-w-sm w-full bg-surface rounded-3xl border border-surface-dark/40 p-8 md:p-10 shadow-2xl shadow-border-main/10 flex flex-col z-10 min-h-[500px]">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <Link to="/settings" className="text-primary/60 hover:text-primary transition-colors" title="Settings">
            <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center">
            <span className="text-primary font-medium tracking-wide animate-pulse">Consulting coach...</span>
          </div>
        ) : error ? (
          <div className="flex-1 flex flex-col justify-center items-center">
            ...
            <button
              ...
              className="bg-primary hover:bg-primary-strong text-on-primary py-2 px-4 rounded-xl text-xs cursor-pointer"
            >
              Try Again
            </button>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-center items-center text-center select-none py-12">
            <svg className="w-12 h-12 text-primary/30 mb-4" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 18.75a6 6 0 006-6v-1.5m-6 7.5a6 6 0 01-6-6v-1.5m6 7.5v3.75m-3.75 0h7.5M12 15.75a3 3 0 01-3-3V4.5a3 3 0 116 0v8.25a3 3 0 01-3 3z" />
            </svg>
            <h3 className="font-serif text-2xl font-normal text-text-main italic mb-2">Tiny Coach is listening</h3>
            <p className="text-xs text-text-muted/80 leading-relaxed mb-6 max-w-[240px]">
              Check in a few times and Tiny Coach will have gentle suggestions for you.
            </p>
            <Link
              to="/habits/new"
              className="bg-primary hover:bg-primary-strong text-on-primary py-3 px-5 rounded-2xl font-medium text-xs tracking-wide transition-all shadow-md shadow-border-main/15 cursor-pointer text-center block w-full"
            >
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              ...
              <div className="text-left mb-6">
                <h1 className="font-serif text-2xl font-normal text-text-main italic leading-tight mb-1">
                  Coaching Notes
                </h1>
                <p className="text-xs text-text-muted/70 font-light">
              ...
              <div className="bg-surface-dark/15 border border-border-main/10 rounded-3xl p-5 text-left mb-6 select-none animate-fadeIn">
                <label className="flex items-start gap-3 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={aiConsent}
                    onChange={(e) => handleToggleConsent(e.target.checked)}
                    className="w-4 h-4 rounded border-border-main/20 text-primary focus:ring-plum-main/30 accent-plum-main mt-0.5 cursor-pointer"
                  />
                  <div>
                    <span className="block text-xs font-semibold text-text-main">
                      Enable AI personalized suggestions
                    </span>
                    <span className="block text-[10px] text-text-muted/75 leading-relaxed mt-0.5">
              ...
              {/* Suggestion Card */}
              {suggestion && (
                <div className="bg-surface-dark/15 border border-border-main/10 rounded-3xl p-6 text-left relative animate-fadeIn min-h-[160px] flex flex-col justify-between gap-4">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    {loadingCoach && (
                      <span className="lowercase font-normal text-[7px] text-text-muted/45 animate-pulse">(thinking...)</span>
                    )}
                  </div>
                  
                  <p className="font-serif italic text-base text-text-main leading-relaxed flex-1">
                    "{suggestion.message}"
                  </p>
                  ...
                  {suggestion.actionLabel && suggestion.actionPath && (
                    <Link
                      ...
                      className="w-full bg-primary hover:bg-primary-strong text-on-primary py-3 px-4 rounded-2xl font-medium text-xs text-center block transition-all duration-200 shadow-md shadow-border-main/10 cursor-pointer"
                    >
            ...
            <nav className="mt-8 pt-4 border-t border-border-main/10 flex justify-around items-center text-xs select-none">
              ...
              <Link to="/coach" className="flex flex-col items-center gap-0.5 cursor-pointer text-accent font-bold transition-colors">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
