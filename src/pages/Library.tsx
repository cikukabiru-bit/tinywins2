      ...
      <div className="relative max-w-sm w-full bg-surface rounded-3xl border border-surface-dark/40 p-8 md:p-10 shadow-2xl shadow-border-main/10 flex flex-col z-10 min-h-[500px]">
        ...
        <header className="flex justify-between items-start mb-6 select-none">
          <div className="text-left">
            ...
            <p className="text-xs text-text-muted/70 font-light">Gentle resources for your journey.</p>
          </div>
          <button
        ...
        {showSettings && (
          <div className="bg-surface-dark/15 border border-border-main/10 rounded-2xl p-4 mb-4 text-left animate-fadeIn select-none">
            <h4 className="text-[10px] uppercase tracking-wider text-text-muted/60 font-bold mb-2 ml-0.5">Library Preferences</h4>
            <label className="flex items-center gap-2 cursor-pointer mb-3">
              ...
              />
              <span className="text-xs font-semibold text-text-main">Include support links on habits</span>
            </label>

            {supportLinksEnabled && (
              <div className="border-t border-border-main/5 pt-2 ml-1">
                <span className="block text-[8px] uppercase tracking-wider text-text-muted/50 font-bold mb-2">Preferred Formats</span>
                <div className="grid grid-cols-2 gap-2">
                  {['video', 'music', 'article', 'podcast', 'prayer'].map((type) => {
                    const checked = supportStyle.includes(type)
                    return (
                      <label key={type} className="flex items-center gap-2 cursor-pointer text-[10px] text-text-main">
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={() => handleToggleStyle(type)}
                          className="w-3 h-3 rounded border-border-main/20 text-primary focus:ring-plum-main/30 accent-plum-main"
                        />
        ...
        <div className="grid grid-cols-2 gap-2 mb-4 select-none">
          <div>
            <label className="block text-[8px] uppercase tracking-wider text-text-muted/50 font-bold mb-1 ml-1 text-left">Format</label>
            <select
              value={filterType}
          ...
          <div>
            <label className="block text-[8px] uppercase tracking-wider text-text-muted/50 font-bold mb-1 ml-1 text-left">Category</label>
            <select
        ...
        {loading ? (
          <div className="flex-1 flex items-center justify-center">
            <span className="text-primary font-medium tracking-wide animate-pulse">Consulting library...</span>
          </div>
        ) : (
            ...
            <div>
              ...
              {filteredItems.length === 0 ? (
                <div className="text-center py-12 select-none">
                  <p className="text-xs text-text-muted/60">No support links match this filter.</p>
                </div>
              ) : (
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        <div className="flex items-center gap-1.5">
                          <span className="text-[7px] font-bold uppercase tracking-wider text-accent bg-accent/5 px-1 py-0.5 rounded border border-accent/10">
                            {item.category}
                          </span>
                      ...
                      <h4 className="font-semibold text-text-main text-xs mb-1">{item.title}</h4>
                      <p className="text-[10px] text-text-muted/85 font-light leading-relaxed mb-3">
                        {item.short_description}
                      </p>

                      <div className="flex justify-between items-center mt-auto pt-2 border-t border-border-main/5">
                        <div className="text-[8px] text-text-muted/60 font-semibold">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <a
                            ...
                            className="bg-primary hover:bg-primary-strong text-on-primary py-1 px-3.5 rounded-lg text-[9px] font-semibold transition-colors text-center inline-block cursor-pointer"
                          >
                            Open
                          </a>
                        </div>
              ...
              {/* Disclaimer */}
              <p className="text-[8px] text-text-muted/50 mt-4 leading-normal select-none italic text-center">
